...
class MemoryService:
    ...
    async def search(
        ...
        try:
            ...
            logger.info(f"Memory search completed: user={user_id}, results={len(results)}")
            return results
    ...
    async def get_stats(self, user_id: str, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Memory stats retrieved: user={user_id}, total={stats.get('total_memories', 0)}")
            return stats
    ...
    async def store(
        ...
        try:
            logger.info(f"Memory store: user={user_id}, session={session_id}, layer={layer}")
            ...
            logger.info(f"Memory stored: user={user_id}, memory_id={result.get('memory_id')}")
            return result
    ...
    async def delete(self, user_id: str, memory_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Memory deleted: user={user_id}, memory_id={memory_id}, success={success}")
            return success
    ...
    async def get_session_memories(
        ...
        try:
            logger.info(f"Session memories request: user={user_id}, session={session_id}")
            ...
            logger.error(f"Session memories error for user {user_id}: {e}", exc_info=True)
            raise
    ...
    async def flush(self, user_id: str, confirm: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self.memory_manager.flush_all
            )
            
            logger.warning(f"Memory flushed: memories={result.get('flushed_memories', 0)}")
            return result
